...
\newcommand{\examinerNOneA}{\item[N1a] Output of \texttt{format\_N1(N1)}}.
...
\newcommand{\examinerNTwoA}{\item[N2a] Output of \texttt{format\_N2(N2)}.}
\newcommand{\examinerNZero}{\item[N0] Problem number, all problems, including those within \texttt{problemblock} objects.}
\newcommand{\examinerNZeroA}{\item[N0a] Output of \texttt{format\_N0(N0)}}
...
\newcommand{\examinerNOneAA}{N1 & Problem number (top level \texttt{problem} or \texttt{problemblock} objects).}
\newcommand{\examinerNOneAAA}{N2 & Output of \texttt{examiner\_opts\$format\_N1(N1)}}.
\newcommand{\examinerNTwoAA}{N2A & \texttt{NULL} if not in a \texttt{problemblock} object. If in a \texttt{problemblock} object, the problem number with that block.}

...
Note that the rendering encloses the output in the environments \texttt{problemset}, \texttt{problemsettext} \texttt{problems}, \texttt{problem}, \texttt{problemtext}, and \texttt{answers}.
...
To shuffle the questions, set the \texttt{format} option \texttt{randomize = TRUE},
<<echo = TRUE, results = 'asis'>>=
cat(format(problems, randomize = TRUE))
@ 
